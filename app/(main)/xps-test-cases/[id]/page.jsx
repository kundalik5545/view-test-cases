...
const XpsTcFilterViewPage = () => {
  ...
  // Determine filter type and value from id param
  const filterConfig = useMemo(() => {
    ...
    const schemeLevelValues = ["TL", "ML", "SL"];

    if (moduleValues.includes(id)) {
      return {
        type: "module",
        value: id,
        label: id === "ToolsAndProcesses" ? "Tools and Processes" : id,
      };
    } else if (schemeLevelValues.includes(id)) {
      return { type: "schemeLevel", value: id, label: id };
    }
    return null;
  }, [id]);
  ...
  useEffect(() => {
    const fetchTestCases = async () => {
      try {
        ...
        if (filterConfig) {
          if (filterConfig.type === "module") {
            params.append("module", filterConfig.value);
          } else if (filterConfig.type === "schemeLevel") {
            params.append("schemeLevel", filterConfig.value);
          }
        }
        ...
        const response = await fetch(
          `/api/test-cases/xps?${params.toString()}`
        );
        if (!response.ok) {
  ...
  const handleViewDetails = (testCaseId) => {
    router.push(`/xps-test-cases/view?testCaseId=${testCaseId}`);
  };
        ...
        <Card>
          <CardContent className="pt-6">
            <p className="text-center text-muted-foreground">
              Invalid filter parameter. Please use a valid module or scheme
              level.
            </p>
          </CardContent>
        </Card>
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <div className="mb-6">
        <h1 className="text-3xl font-bold mb-2">
          XPS Test Cases - {filterConfig.label}
        </h1>
        <p className="text-muted-foreground">
          Filtered by{" "}
          {filterConfig.type === "module" ? "Module" : "Scheme Level"}:{" "}
          {filterConfig.label}
        </p>
      </div>
                ...
                <tbody>
                  {testCases.map((testCase) => (
                    ...
                    >
                      ...
                      <td className="p-3 text-sm">{testCase.module || "—"}</td>
                      <td className="p-3 text-sm">
...
export default XpsTcFilterViewPage;
