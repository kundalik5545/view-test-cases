...
export async function POST(request) {
  try {
    ...
    // Get test case to access portal
    const testCase = await prisma.eMemberTestCase.findUnique({
      where: { id },
    });
    ...
    const portal = testCase.portal || "Unknown";
    const sanitizedTestCaseId = testCaseId.replace(/[^a-zA-Z0-9]/g, "_");
    const sanitizedPortal = portal.replace(/[^a-zA-Z0-9]/g, "_");

    let filename = `${sanitizedTestCaseId}_${sanitizedPortal}_${timestamp}.png`;
    let filePath = path.join(SCREENSHOTS_DIR, filename);
    let counter = 2;

    // Check if file exists and increment counter
    while (existsSync(filePath)) {
      filename = `${sanitizedTestCaseId}_${sanitizedPortal}_${timestamp}_${counter}.png`;
    ...
    try {
      const updated = await prisma.eMemberTestCase.update({
        where: { id },
        data: {
...
export async function DELETE(request) {
  try {
    ...
    // Update database
    await prisma.eMemberTestCase.update({
      where: { id },
      data: {
